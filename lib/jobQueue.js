...
export class JobQueue {
  ...
  async scheduleJob(feedId, type = 'scheduled') {
    console.log(`🔍 Checking if job can be scheduled for feed ${feedId}`);
    
    // Double-check: Get all active jobs for this feed
    const activeJobs = await SyncJobModel.getActive(feedId);
    if (activeJobs.length > 0) {
      console.log(`❌ Cannot schedule job - found ${activeJobs.length} active jobs for feed ${feedId}`);
      activeJobs.forEach(job => {
        console.log(`  - Job ${job.id}: ${job.status} (created: ${job.createdAt})`);
      });
      throw new Error(`A sync job is already active for this feed (${activeJobs.length} jobs found)`);
    }
