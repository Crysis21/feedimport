...
export class BatchFeedProcessor {
  ...
  async processBatch(jobId, batch) {
    ...
    for (const product of batch.products) {
      ...
      if (processedIds.has(productKey)) {
        console.log(`⏭️ Skipping duplicate product in batch: ${product.title} (${productKey})`);
        skippedCount++;
        continue;
      }
    ...
    console.log(`📊 Batch ${batch.index + 1} complete: ${processedCount} saved, ${failedCount} failed, ${skippedCount} skipped`);
  ...
  extractProductData(item) {
    ...
    console.log(`📦 Batch extracted product with ${Object.keys(product).length} fields:`, {
      id: product.id,
      sku: product.sku,
