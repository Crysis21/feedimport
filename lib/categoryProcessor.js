    ...
    while (retryCount < this.maxRetries) {
      try {
        ...
        for (const result of results) {
          ...
          console.log(`🔍 Updating product - Document ID: ${product.documentId}, Feed ID: ${product.feedId}, Product ID: ${product.productId}`);
          
          try {
            if (result.categoryMatch) {
              await ProductModel.updateCategory(product.documentId, {
            ...
            console.error(`❌ Database error for ${product.documentId}:`, dbError.message);
            failed++;
          }
        }
        ...
        if (retryCount >= this.maxRetries) {
          ...
          for (const product of products) {
            console.log(`🔍 Fallback updating product - Document ID: ${product.documentId}, Feed ID: ${product.feedId}`);
            try {
              await ProductModel.updateCategory(product.documentId, {
                title: 'Uncategorized',
                id: 0,
              ...
              console.error(`❌ Database fallback error for ${product.documentId}:`, dbError.message);
            }
          }
        } else {
          // Wait before retry
